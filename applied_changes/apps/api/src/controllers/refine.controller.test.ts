...
import { createOpenAIConfigError } from "../lib/errors";
...
vi.mock("../services/prompts", () => ({
  ...
  buildRefinePrompt: vi.fn(
    (request) => `Refine content: ${request.instruction}`,
  ),
}));
...
describe("refineController", () => {
  const mockRequest: RefineRequest = {
  ...
  describe("refineContent", () => {
    it("should refine content stream successfully", async () => {
      ...
      vi.mocked(streamCompletion).mockReturnValue(mockGenerator as any);

      const result = await refineController.refineContent(
        mockRequest,
        mockContext,
      );
    ...
    it("should throw error when API key is missing", async () => {
      ...
      await expect(
        refineController.refineContent(mockRequest, mockContextWithoutKey),
      ).rejects.toThrow();
    ...
    it("should wrap stream errors appropriately", async () => {
      ...
      await expect(
        refineController.refineContent(mockRequest, mockContext),
    ...
    it("should use default model when not specified", async () => {
      ...
      await refineController.refineContent(
        mockRequest,
        mockContextWithoutModel,
      );
    ...
    it("should handle requests with context", async () => {
      const requestWithContext: RefineRequest = {
    ...
    it("should handle different instructions", async () => {
      const differentRequest: RefineRequest = {
