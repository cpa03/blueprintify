...
var TechStackCategory = external_exports.enum([
  "frontend",
  "backend",
  "database",
  "hosting",
...
var TechStackItem = external_exports.object({
  name: external_exports.string().min(1),
  category: TechStackCategory,
  version: external_exports.string().optional()
});
var BlueprintRequestSchema = external_exports.object({
  projectName: external_exports.string().min(1, "Project name is required").max(100),
  description: external_exports.string().min(10, "Description must be at least 10 characters").max(2e3),
  techStack: external_exports.array(TechStackItem).min(1, "At least one technology is required"),
  features: external_exports.array(external_exports.string().min(1)).optional(),
  targetAudience: external_exports.string().optional(),
  constraints: external_exports.string().optional()
});
var TaskGenerationRequestSchema = external_exports.object({
...
var TemplateSchema = external_exports.object({
  ...
  defaultDescription: external_exports.string(),
  techStack: external_exports.array(TechStackItem),
  features: external_exports.array(external_exports.string())
});
var StreamChunkSchema = external_exports.object({
  type: external_exports.enum(["content", "error", "done"]),
  content: external_exports.string().optional(),
  error: external_exports.string().optional()
});
var GenerationResultSchema = external_exports.object({
  blueprint: external_exports.string(),
  tasks: external_exports.string().optional(),
  generatedAt: external_exports.string()
});

// src/services/prompts.ts
var ARCHITECT_SYSTEM_PROMPT = `You are Agent 00, a Principal Software Architect with 20+ years of experience designing scalable, maintainable systems. Your role is to create comprehensive architectural documentation that enables autonomous development agents to build the project from scratch.
...
var TASK_SPLITTER_SYSTEM_PROMPT = `You are a Technical Project Manager specializing in breaking down architectural plans into actionable development tasks. You excel at:
...
var REFINER_SYSTEM_PROMPT = `You are an expert technical editor. Your job is to improve specific sections of documentation based on user feedback. You:
...
function buildBlueprintPrompt(request) {
...
__name(buildBlueprintPrompt, "buildBlueprintPrompt");
function buildTaskPrompt(blueprint, projectName) {
...
function buildRefinePrompt(request) {
...
__name(buildRefinePrompt, "buildRefinePrompt");
...
__name(createAIClient, "createAIClient");
...
function createStreamFromGenerator(generator, onComplete) {
  const encoder = new TextEncoder();
