...
## Cycle 336 (2026-08-03 — Security Engineer: dependabot eslint 10.8.0 PR audit — bump REVERTED (invalid peer tree: `eslint-plugin-jsx-a11y@6.10.2`/`eslint-plugin-react@7.37.5` do not support ESLint 10; `npm ls` exit 1, 12 ELSPROBLEMS); no secrets introduced; `npm audit` 0 vulns; eslint restored to 9.39.5 — gates green lint ✅ npm ls ✅ audit ✅ secrets ✅)

> **Security Engineer (2026-08-03)**: Audited dependabot PR `dependabot/npm_and_yarn/eslint-10.8.0` (eslint 9.39.5 → 10.8.0, changed files: `package.json` + `package-lock.json` only). **Verdict: bump NOT safe to merge — reverted to 9.39.5.** Root cause: ESLint 10 is a major-version bump whose peer-dependency graph is not yet supported by this repo's plugins — `eslint-plugin-jsx-a11y@6.10.2` declares peer `eslint: ^3 || ^4 || ^5 || ^6 || ^7 || ^8 || ^9` and `eslint-plugin-react@7.37.5` declares peer `eslint: ^3 || ^4 || ^5 || ^6 || ^7 || ^8 || ^9.7` (neither includes `^10`). With the bump applied, `npm ls --all` exits **1 with 12 ELSPROBLEMS** (`invalid: eslint@10.8.0`), violating the repo's quality gate (`npm ls` exit 0). Also confirmed `@eslint/js` stayed pinned at 9.39.5 under the bump (major skew), and aligning it to `@eslint/js@10.0.1` surfaced 3 pre-existing `no-useless-assignment` lint errors in files outside this PR (`apps/web/src/components/TemplateGrid.tsx`, `apps/web/src/components/editor/EditorHeader.tsx`, +1) — i.e., the codebase is not yet ESLint-10-clean, and merging would either break CI or silently skip plugin rules. **No secrets introduced** (diff grep + `npm run scan:secrets` clean, 311 files). **No new CVEs** (`npm audit` 0 vulns in both states). **No deprecated functions introduced** (0 deprecated packages in eslint-related tree). **Action**: reverted `package.json`/`package-lock.json` to `origin/main` (eslint 9.39.5) and verified gates: lint ✅ (0 errors/0 warnings), `npm ls` ✅ exit 0, `npm audit` ✅ 0 vulns, secrets scan ✅. **Recommendation**: re-apply the eslint 10 bump only after `eslint-plugin-jsx-a11y` and `eslint-plugin-react` release versions declaring `eslint: ^10` peer support, and after the 3 `no-useless-assignment` code sites are cleaned up. Do NOT enable `legacy-peer-deps` to force the install — it masks an unsupported dependency tree (the existing `.npmrc` `legacy-peer-deps=true` was added for this same bump and should be removed when the bump is re-attempted properly).
