# BroCula Audit — 2026-08-18 Run 78

**Branch**: `brocula/loop-2026-08-18-run-78`
**Date**: 2026-08-18
...
| LH Performance (Prod) | **99 hunt / 99 warm** ⭐ (hunt pass FCP/LCP 1.7s, TBT 10ms; warm re-pass FCP/LCP 1.6s, TBT 60ms, **0 savings>0 audits** both passes — sub-100 both passes = ARM64 runner CPU variance per Run 67/70/71/73/74/75 precedent) |
| LH Accessibility | **100** 🏆 |
| LH Best Practices | **100** 🏆 |
| LH SEO | **100** 🏆 |
| Optimization Opportunities | **0 actionable** ✅ |
| Tests | **2,590 pass** (1,203 web + 535 api + 852 shared) ✅ |
| Quality Gates | All pass ✅ |

## Changes in This Run

- **No application code changes required.** BroCula hunt (build → preview → console capture → Lighthouse) returned **0 errors, 0 warnings, 0 failed non-API requests**, and Lighthouse **99 hunt / 99 warm — 100-100-100** (hunt pass FCP/LCP 1.7s, TBT 10ms, SI 1.7s, TTI 2.3s; warm re-pass FCP/LCP 1.6s, TBT 60ms, CLS 0.007, identical 225 KiB byte weight, **0 audits with `overallSavingsMs > 0`** on both passes — no code-level action warranted). The interactive sweep (`scripts/brocula-sweep.mjs`, **25/25 assertions**) returned **0 console errors / 0 warnings / 0 failed non-API requests** (only expected `/api/generate` failures during the API-unavailable error-path test). Code is clean; no fixes or optimizations needed.
- **Playwright Chromium re-installed** on this runner (`npx playwright install chromium`) — full Chromium + headless shell binaries were missing on the fresh runner (same as Run 76/77 precedent).
...
**0 audits with `overallSavingsMs > 0`** across both passes — no actionable optimization opportunities. Hunt pass FCP/LCP 1.7s, TBT 10ms; warm re-pass FCP/LCP 1.6s, TBT 60ms with identical 225 KiB byte weight. Performance 99 on both passes = ARM64 runner CPU variance, consistent with Run 67/70/71/73/74/75 precedent.

## Lighthouse Diagnostics (hunt pass / warm re-pass)

| Metric | Hunt | Warm |
|---|---|---|
| First Contentful Paint | 1.7s | 1.6s |
| Largest Contentful Paint | 1.7s | 1.6s |
| Total Blocking Time | 10ms | 60ms |
| Speed Index | 1.7s | — |
...
> **Note (environment artifact, not a code defect):** the first sweep attempt's `ERR_CONNECTION_REFUSED` chunk loads (`Toast-*.js`, `ErrorFallback-*.js`) were the preview-server teardown race (hunt cleanup killed the serving process mid-sweep) — resolved on a clean re-sweep with a freshly started preview server, zero errors/warnings (Run 73/74/75/76 precedent). The only console errors observed on the clean sweep were the expected `/api/generate` failures (502 Bad Gateway) during the API-unavailable error-path test — the API server is not running in the audit environment and the preview/dev proxy surfaces the failure as a 502 response. Zero non-API request failures.
...
|---|---|---|
| Performance | 99 hunt / 99 warm ⭐ | Hunt FCP/LCP 1.7s, TBT 10ms; warm FCP/LCP 1.6s, TBT 60ms, CLS 0.007, 0 savings audits (sub-100 both passes = ARM64 runner CPU variance per Run 67/70/71/73/74/75 precedent) |
