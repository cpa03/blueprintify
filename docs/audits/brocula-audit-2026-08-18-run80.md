# BroCula Audit — 2026-08-18 Run 80

**Branch**: `brocula/loop-2026-08-18-run-80`
**Date**: 2026-08-18
...
| LH Performance (Prod) | **99 hunt / 99 warm** ⭐ (hunt pass FCP/LCP 1.6s, TBT 60ms; warm re-passes FCP/LCP 1.7s, TBT 60–70ms, **0 savings>0 audits across all three passes** — 99 = ARM64 runner CPU variance per Run 67/70/71/73/74/75/76/78/79 precedent) |
...
**0 audits with `overallSavingsMs > 0`** across all three passes — no actionable optimization opportunities. Hunt pass FCP/LCP 1.6s, TBT 60ms, SI 1.6s, TTI 2.4s; warm re-passes FCP/LCP 1.7s, TBT 60–70ms with identical 225 KiB byte weight. Performance 99 on all passes = ARM64 runner CPU variance, consistent with Run 67/70/71/73/74/75/76/78/79 precedent.

## Lighthouse Diagnostics (hunt pass / warm re-pass)

| Metric | Hunt | Warm |
|---|---|---|
| First Contentful Paint | 1.6s | 1.7s |
| Largest Contentful Paint | 1.6s | 1.7s |
...
> **Note (environment artifact, not a code defect):** the first sweep attempt's `ERR_CONNECTION_REFUSED` chunk loads (`Toast-*.js`, `ErrorFallback-*.js`) were the preview-server teardown race (the hunt's orphaned `vite preview` child held port 4173, then died mid-sweep) — resolved on a clean re-sweep with a freshly started preview server, zero errors/warnings (Run 73/74/75/76/78/79 precedent). The only console errors observed on the clean sweep were the expected `/api/generate` failures (502 Bad Gateway) during the API-unavailable error-path test — the API server is not running in the audit environment and the preview/dev proxy surfaces the failure as a 502 response. Zero non-API request failures.
...
|---|---|---|
| Performance | 99 hunt / 99 warm ⭐ | Hunt FCP/LCP 1.6s, TBT 60ms; warm FCP/LCP 1.7s, TBT 60–70ms, CLS 0.007, 0 savings audits (99 = ARM64 runner CPU variance per Run 67/70/71/73/74/75/76/78/79 precedent) |
