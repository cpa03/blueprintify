...
**Preferred approach**: Use `node-version-file: ".node-version"` instead of hardcoding `"22"`. This keeps CI in sync automatically as the project evolves (`.node-version` contains `22`). An alternative is to hardcode `"22"` (simpler change, verified working).
...
| ULW-7 | 2026-06-14 | Sisyphus ULW Loop re-applied using \`node-version-file: ".node-version"\` approach (auto-sync with project .node-version), fixed stale doc refs, typecheck ✅ lint ✅ build ✅ tests 1289/1289 ✅, push rejected (no \`workflows: write\`). Commit 21d1e45 (unpushed, soft-reset). Also resolved 9 duplicate CI issues by consolidating into single fix. |
