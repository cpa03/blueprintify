// Wizard configuration constants
// Source of truth for wizard-related constants

import { ENV } from "../env";
import {
  SHARED_DEFAULTS,
  WIZARD_STEP_KEYS,
  EXPORT_FILENAMES,
  EXPORT_DEFAULTS,
} from "@blueprint/shared";

/**
 * Wizard review edit shortcut keys
 * Maps editable wizard sections to their Alt+keyboard shortcuts shown on the review step.
 * Flexy says: No hardcoded "1"/"2"/"3" strings in StepReview keyboard handler!
 */
export const WIZARD_REVIEW_EDIT_SHORTCUTS = {
  INFO: "1" as const,
  STACK: "2" as const,
  FEATURES: "3" as const,
} as const;

/**
 * Wizard step configuration
 * Defines the ordered steps in the project setup wizard
 * Flexy says: Step keys reference WIZARD_STEP_KEYS from @blueprint/shared!
 */
export const WIZARD_STEPS = [
  { key: WIZARD_STEP_KEYS.INFO, label: "Project Info", icon: "document" as const, shortcut: "1" },
  { key: WIZARD_STEP_KEYS.STACK, label: "Tech Stack", icon: "sliders" as const, shortcut: "2" },
  { key: WIZARD_STEP_KEYS.FEATURES, label: "Features", icon: "sparkles" as const, shortcut: "3" },
  { key: WIZARD_STEP_KEYS.REVIEW, label: "Review", icon: "eye" as const, shortcut: "4" },
  {
    key: WIZARD_STEP_KEYS.GENERATING,
    label: "Generate",
    icon: "lightning" as const,
    shortcut: "5",
  },
] as const;

/**
 * Tech stack category icons
 * Maps category keys to their emoji representations
 */
export const CATEGORY_ICONS: Record<string, string> = {
  frontend: "🎨",
  backend: "⚙️",
  database: "🗄️",
  hosting: "☁️",
  styling: "🖌️",
  ai: "🤖",
  testing: "🧪",
  other: "📦",
} as const;

/**
 * UI emoji constants - centralized for consistency
 * Used across components for consistent emoji rendering
 */
export const UI_EMOJIS = {
  /** Document/editing icon */
  DOCUMENT: "📝",
  /** Blank document icon */
  DOCUMENT_BLANK: "📄",
  /** Settings/gear icon */
  SETTINGS: "⚙️",
} as const;

/**
 * Minimum requirements for wizard validation
 * Defines the minimum counts/values for a valid project configuration
 */
export const MIN_REQUIREMENTS = {
  TECH_STACK: 1,
  PROJECT_NAME: 1,
  DESCRIPTION: 10,
} as const;

/**
 * Export configuration
 * Settings for ZIP compression, file naming, and export behavior
 */
export const EXPORT_CONFIG = {
  ZIP_COMPRESSION_LEVEL: EXPORT_DEFAULTS.ZIP_COMPRESSION_LEVEL,
  ZIP_COMPRESSION: EXPORT_DEFAULTS.ZIP_COMPRESSION,
  JSON_INDENT: EXPORT_DEFAULTS.JSON_INDENT,
  COPY_TEXTAREA_OFFSET: EXPORT_DEFAULTS.COPY_TEXTAREA_OFFSET_PX,
  README_FILENAME: EXPORT_DEFAULTS.README_FILENAME,
  METADATA_FILENAME: EXPORT_DEFAULTS.METADATA_FILENAME,
  BLUEPRINT_FILENAME: EXPORT_FILENAMES.BLUEPRINT,
  TASK_FILENAME: EXPORT_FILENAMES.TASKS,
  METADATA_VERSION: SHARED_DEFAULTS.API_VERSION,
  DOCS_FOLDER: EXPORT_DEFAULTS.DOCS_FOLDER,
  ZIP_FILENAME_SUFFIX: EXPORT_DEFAULTS.ZIP_FILENAME_SUFFIX,
  DATE_FORMAT_SEPARATOR: EXPORT_DEFAULTS.DATE_FORMAT_SEPARATOR,
} as const;

/**
 * Suggested features for quick add
 * Pre-defined feature suggestions displayed in the features step
 */
export const SUGGESTED_FEATURES = [
  "User authentication",
  "Database integration",
  "API endpoints",
  "Real-time updates",
  "File uploads",
  "Search functionality",
  "Email notifications",
  "Admin dashboard",
  "Payment processing",
  "Analytics tracking",
] as const;

/**
 * Editor tab constants
 * Identifiers for the split-pane editor tabs
 * Single source of truth: @blueprint/shared
 * Flexy says: No hardcoded "blueprint"/"tasks" in editor components!
 */
export { EDITOR_TABS } from "@blueprint/shared";

/**
 * Generates a README template for project documentation
 * @param projectName - The name of the project
 * @returns Formatted README markdown string
 */
export const README_TEMPLATE = (projectName: string): string => {
  const homepageUrl = ENV.PROJECT_HOMEPAGE_URL;
  return `# ${projectName} - Documentation

This folder contains AI-generated documentation for your project.

## Files

- **${EXPORT_FILENAMES.BLUEPRINT}** - Architectural blueprint and technical specifications
- **${EXPORT_FILENAMES.TASKS}** - Prioritized task breakdown for implementation

## Usage

These files are designed for use with autonomous development agents.
Place this .docs folder in your project root to enable agent-assisted development.

---
Generated by [${ENV.APP_NAME}](${homepageUrl})
Generated at: ${new Date().toISOString()}
`;
};

/** Default project name fallback */
export const DEFAULT_PROJECT_NAME = ENV.DEFAULT_PROJECT_NAME;

/**
 * Tech stack project type detection rules
 * Maps technology names to project type identifiers for export scaffolding
 * Flexy says: No hardcoded tech names - everything in config!
 */
export const TECH_STACK_DETECTION = {
  /** Technologies that indicate a React/Next.js project */
  REACT_INDICATORS: ["react", "next.js"] as const,
  /** Technologies that indicate a Node.js/Express project */
  NODE_INDICATORS: ["node.js", "express", "hono"] as const,
  /** Technologies that indicate a Python/Django project */
  PYTHON_INDICATORS: ["python", "django", "flask"] as const,
} as const;
