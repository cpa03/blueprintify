...
permissions:
  bash:
    ...
    "*": allow
memory:
  - ${AGENT_MEMORY_DIR:-.agent/memory}/backend.md
---
...
- **Memory Ingestion**: Before starting, you MUST read `${AGENT_MEMORY_DIR:-.agent/memory}/backend.md` if it exists. This file contains lessons learned from previous tasks.
...
- **Consult System Memory**: Read `${AGENT_MEMORY_DIR:-.agent/memory}/backend.md` to avoid repeating past mistakes.
- **Check Existing Code**: Use file search or grep / text search to avoid duplicating logic.
