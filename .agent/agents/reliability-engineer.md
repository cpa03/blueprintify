...
permissions:
  bash:
    ...
    "*": allow
memory:
  - ${AGENT_MEMORY_DIR:-.agent/memory}/reliability.md
---
...
- **Memory Ingestion**: Before starting, read `${AGENT_MEMORY_DIR:-.agent/memory}/reliability.md` (or general backend/frontend memory).
...
- **Verify**: Use text search (grep) to ensure usage patterns are updated.
