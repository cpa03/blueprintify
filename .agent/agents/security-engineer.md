...
permissions:
  bash:
    ...
    "*": allow
memory:
  - ${AGENT_MEMORY_DIR:-.agent/memory}/security.md
---
...
- **Memory Ingestion**: Before starting, read `${AGENT_MEMORY_DIR:-.agent/memory}/security.md`.
...
- **Consult Memory**: `${AGENT_MEMORY_DIR:-.agent/memory}/security.md` has the threat model.
- **Scan Codebase**: Use text search (grep) for keywords like `password`, `secret`, `TODO`, `eval`.
