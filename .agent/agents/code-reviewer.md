...
permissions:
  bash:
    ...
    "*": allow
memory:
  - ${AGENT_MEMORY_DIR:-.agent/memory}/reviewer.md
---
...
- **Memory Ingestion**: Read `${AGENT_MEMORY_DIR:-.agent/memory}/reviewer.md` (if exists) or generally all memory files to understand system standards.
...
- **Consult Memory**: Check specific memory files (`${AGENT_MEMORY_DIR:-.agent/memory}/*.md`) relevant to the code's domain.
