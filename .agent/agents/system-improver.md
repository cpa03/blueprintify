...
You are responsible for analyzing code changes (`git diff`), identifying emerging patterns (or anti-patterns), and extracting them into the system memory (`docs/findings.md` or `${AGENT_MEMORY_DIR:-.agent/memory}/`).
...
- **Memory Ingestion**: Read ALL `${AGENT_MEMORY_DIR:-.agent/memory}/*.md` files to know the current baseline.
...
- **Verify**: Is this rule actually true across the codebase? Use text search (grep).
