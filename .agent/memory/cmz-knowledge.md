...
- `${AGENT_MODEL:-deepseek-v4-flash-free}` (exclusive per AGENTS.md mandate)
...
1. Standardize on `${AGENT_MODEL:-deepseek-v4-flash-free}` exclusively
...
- **Mandatory**: Use `${AGENT_MODEL:-deepseek-v4-flash-free}` exclusively per AGENTS.md mandate
...
- Always use `${AGENT_MODEL:-deepseek-v4-flash-free}` exclusively per AGENTS.md mandate
- **NO fallback models allowed** - AGENTS.md mandates exclusive use of `${AGENT_MODEL:-deepseek-v4-flash-free}`
...
- Store knowledge in ${AGENT_MEMORY_DIR:-.agent/memory}/
...
- Verified all 25 agent skills in `${AGENT_ROOT:-.agent}/skill` including `madappgang-claude-code-debugging-strategies` and `vasilyu1983-ai-agents-public-git-commit-message`.
