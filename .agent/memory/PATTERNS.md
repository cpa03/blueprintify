...
- [Agent] All agents MUST use `${AGENT_MODEL:-deepseek-v4-flash-free}` model per AGENTS.md mandate.
- [Agent] No `fallback_models` allowed - AGENTS.md mandates exclusive use of `${AGENT_MODEL:-deepseek-v4-flash-free}`.
...
- [Docs] Keep documentation synchronized with agent model standard (`${AGENT_MODEL:-deepseek-v4-flash-free}`).
...
- [Agent] Do not use inconsistent model references in agent definitions. All agents MUST use `${AGENT_MODEL:-deepseek-v4-flash-free}` per AGENTS.md mandate.
...
- Centralized configuration in `${AGENT_ROOT:-.agent}/`.
